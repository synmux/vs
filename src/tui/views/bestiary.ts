...
export class BestiaryView extends ListDetailView {
  ...
  protected buildItems(): ListItem[] {
    return this.entries().map((entry) => ({ key: entry.pageName, name: entry.name, description: entry.type }));
  }

  protected renderDetail(key: string): DetailContent {
    const entry = this.entries().find((candidate) => candidate.pageName === key);
    if (!entry) return { text: "" };
    const lines: string[] = [entry.name];
    if (entry.number) lines.push(`Bestiary No. ${entry.number}`);
    if (entry.type) lines.push(`Type: ${entry.type}`);
    if (entry.dlc) lines.push(`DLC: ${entry.dlc}`);
