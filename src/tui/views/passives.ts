...
export class PassivesView extends ListDetailView {
  ...
  protected buildItems(): ListItem[] {
    return this.repo.passives().map((passive) => ({
      key: passive.pageName,
      name: passive.name,
      description: passive.dlc,
    }));
  }

  protected renderDetail(key: string): DetailContent {
    const passive = this.repo
      .passives()
    ...
    if (passive.dlc) {
      lines.push(`DLC: ${passive.dlc}`);
    }
    if (passive.description) {
      lines.push("", passive.description);
    }

    const recipes =
      this.repo.evolutionGraph().byPassive.get(passive.pageName) ?? [];
    const links: LinkTarget[] = [];
    if (recipes.length > 0) {
      lines.push(
        "",
        `Enables ${recipes.length} evolution${recipes.length === 1 ? "" : "s"}:`
      );
