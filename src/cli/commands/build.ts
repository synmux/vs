...
export function buildCommand(
  ...
  for (const token of tokens) {
    ...
    if (weaponExact && weapon) {
      owned.weapons.add(weapon.pageName);
    } else if (passiveExact && passive) {
      owned.passives.add(passive.pageName);
    } else if (weapon) {
      owned.weapons.add(weapon.pageName);
    } else if (passive) {
      owned.passives.add(passive.pageName);
  ...
  if (unresolved.length > 0) {
    lines.push(`Unrecognised: ${unresolved.join(", ")}`, "");
  }

  lines.push(`Achievable now (${plan.achievable.length}):`);
  if (plan.achievable.length === 0) {
    lines.push("  (none)");
  }
  ...
  for (const entry of plan.oneAway.slice(0, ONE_AWAY_LIMIT)) {
    lines.push(
      `  ${entry.recipe.result}  (need: ${entry.missing.map((item) => item.key).join(", ")})`
    );
  }
  if (plan.oneAway.length > ONE_AWAY_LIMIT) {
    lines.push(`  …and ${plan.oneAway.length - ONE_AWAY_LIMIT} more`);
  }
