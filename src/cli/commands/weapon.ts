...
export function weaponCommand(repo: Repository, name: string, options: OutputOptions = {}): string {
  ...
  if (options.json) return JSON.stringify(weapon, null, 2);
  ...
  if (evolvesInto.length > 0) {
    ...
    for (const recipe of evolvesInto) lines.push(`  ${recipe.result} (with ${recipe.requiredPassives.join(" + ")})`);
  }
  const madeFrom = recipesForResult(graph, weapon.pageName);
  if (madeFrom.length > 0) {
    lines.push("", "Made from:");
    for (const recipe of madeFrom) lines.push(`  ${recipe.bases.join(" + ")} + ${recipe.requiredPassives.join(" + ")}`);
  }
  return lines.join("\n");
}
