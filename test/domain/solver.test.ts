...
describe("solver (real fixture data)", () => {
  test("recipesForResult returns the recipes that produce a weapon", () => {
    expect(
      recipesForResult(graph, "Bloody Tear").some((recipe) =>
        recipe.bases.includes("Whip")
      )
    ).toBe(true);
  });
  ...
  test("evolutionsForBase returns the recipes a base participates in", () => {
    expect(
      evolutionsForBase(graph, "Whip").some(
  ...
  test("the two directions reference the same recipe", () => {
    const viaBase = evolutionsForBase(graph, "Whip").find(
      (recipe) => recipe.result === "Bloody Tear"
    );
    const viaResult = recipesForResult(graph, "Bloody Tear").find((recipe) =>
      recipe.bases.includes("Whip")
    );
  ...
  test("recipesForPassive returns recipes requiring that passive", () => {
    expect(
      recipesForPassive(graph, "Hollow Heart").every((recipe) =>
        recipe.requiredPassives.includes("Hollow Heart")
      )
    ).toBe(true);
  });
