...
describe("resolveByName", () => {
  test("an exact name match wins, case-insensitively", () => {
    expect(resolveByName(weapons, "whip", (weapon) => weapon.name)?.name).toBe(
      "Whip"
    );
  });
  ...
  test("falls back to a fuzzy match", () => {
    expect(
      resolveByName(weapons, "bloodytear", (weapon) => weapon.name)?.name
    ).toBe("Bloody Tear");
  });

  test("returns undefined when nothing matches", () => {
    expect(
      resolveByName(weapons, "zzzzzz", (weapon) => weapon.name)
    ).toBeUndefined();
  });

  test("returns undefined for an empty query", () => {
    expect(
      resolveByName(weapons, "  ", (weapon) => weapon.name)
    ).toBeUndefined();
  });
});
