...
describe("planBuild (real fixture data)", () => {
  test("a recipe with every base and passive owned is achievable", () => {
    const owned = { weapons: new Set(["Whip"]), passives: new Set(["Hollow Heart"]) };
    expect(planBuild(graph, owned).achievable.some((recipe) => recipe.result === "Bloody Tear")).toBe(true);
  });
  ...
  test("missing exactly one required item lands it in one-away, naming the item", () => {
    ...
    const bloodyTear = plan.oneAway.find((entry) => entry.recipe.result === "Bloody Tear");
    expect(bloodyTear?.missing).toEqual([{ kind: "passive", key: "Hollow Heart" }]);
  });
  ...
  test("a multi-base recipe requires ALL bases (AND), not any", () => {
    ...
    const owned = { weapons: new Set(["Phiera Der Tuphello"]), passives: new Set(["Tirajisú"]) };
    const phieraggi = planBuild(graph, owned).oneAway.find((entry) => entry.recipe.result === "Phieraggi");
    expect(phieraggi?.missing).toEqual([{ kind: "weapon", key: "Eight The Sparrow" }]);
  });
});
...
describe("missingForRecipe and passiveMax", () => {
  ...
  test("without max-level tracking, owning the passive satisfies even passiveMax recipes", () => {
    const recipe = graph.recipes.find((entry) => entry.passiveMax);
    const owned = { weapons: new Set(recipe!.bases), passives: new Set(recipe!.requiredPassives) };
