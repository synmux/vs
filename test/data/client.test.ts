...
import { BucketError, fetchBucket, parseBucketResponse } from "../../src/data/bucket/client.ts";
import { BucketQuery } from "../../src/data/bucket/query.ts";

describe("parseBucketResponse", () => {
  test("returns the bucket rows array", () => {
    expect(parseBucketResponse({ bucket: [{ a: 1 }, { a: 2 }] })).toEqual([{ a: 1 }, { a: 2 }]);
  });
  ...
  test("throws BucketError when the response carries an error string", () => {
    expect(() => parseBucketResponse({ error: "Query timed out" })).toThrow(BucketError);
  });
...
describe("fetchBucket", () => {
  test("calls the bucket endpoint with the encoded query and a User-Agent, returning rows", async () => {
    ...
    const fakeFetch = (async (url: string | URL, init?: RequestInit) => {
      ...
      return new Response(JSON.stringify({ bucket: [{ page_name: "Whip" }] }), { status: 200 });
  ...
  test("throws BucketError on a non-OK HTTP status", () => {
    ...
    const fakeFetch = (async () => new Response("oops", { status: 500 })) as unknown as typeof fetch;
    expect(fetchBucket(query, { fetchImpl: fakeFetch })).rejects.toThrow(BucketError);
  });
});
