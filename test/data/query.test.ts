...
describe("BucketQuery.toLua", () => {
  test("builds a whole-table select with a default limit of 5000", () => {
    expect(
      BucketQuery.table("infobox_weapon").select("page_name", "name").toLua()
    ).toBe(
  ...
  test("includes a where clause", () => {
    expect(
      BucketQuery.table("passive_evolutions")
        .select("evolution")
        .where("evolution", "Bloody Tear")
        .toLua()
    ).toBe(
      "bucket('passive_evolutions').select('evolution').where('evolution','Bloody Tear').limit(5000).run()"
    );
  });

  test("chains multiple where clauses in order (AND)", () => {
    expect(
      BucketQuery.table("t").select("a").where("x", "1").where("y", "2").toLua()
    ).toBe(
  ...
  test("clamps the limit to the 5000 maximum", () => {
    expect(BucketQuery.table("t").select("a").limit(99_999).toLua()).toBe(
      "bucket('t').select('a').limit(5000).run()"
    );
  });
