...
describe("normalizeRow", () => {
  ...
  test("treats a present empty-string boolean as true (presence = true)", () => {
    ...
    expect(
      normalizeRow<NormalizedEvolutionRow>(row, evolutionSpec).passive_max
    ).toBe(true);
  });

  test("treats an omitted boolean as false", () => {
    expect(normalizeRow<{ gift: boolean }>({}, { gift: "bool" })).toEqual({
      gift: false,
    });
  });
  ...
  test("keeps array values for string[] fields", () => {
    ...
    expect(
      normalizeRow<{ base_weapon: string[] }>(row, { base_weapon: "string[]" })
        .base_weapon
    ).toEqual(["Phiera Der Tuphello", "Eight The Sparrow"]);
  });

  test("wraps a scalar into an array for string[] fields", () => {
    expect(
      normalizeRow<{ base_weapon: string[] }>(
        { base_weapon: "Whip" },
        { base_weapon: "string[]" }
      ).base_weapon
    ).toEqual(["Whip"]);
  ...
  test("coerces numbers and defaults missing numerics to 0", () => {
    ...
    expect(normalizeRow<Row>({ cost: 10, move_speed: 1.4 }, spec)).toEqual({
      cost: 10,
      move_speed: 1.4,
    });
  ...
  test("parses string-encoded numbers", () => {
    expect(
      normalizeRow<{ cost: number }>({ cost: "7" }, { cost: "int" })
    ).toEqual({ cost: 7 });
  });

  test("truncates floats for int fields", () => {
    expect(normalizeRow<{ n: number }>({ n: 3.9 }, { n: "int" })).toEqual({
      n: 3,
    });
  });

  test("coerces non-string scalars to string for string fields", () => {
    expect(normalizeRow<{ n: string }>({ n: 5 }, { n: "string" })).toEqual({
      n: "5",
    });
  });
  ...
  test("includes only fields declared in the spec", () => {
    expect(
      normalizeRow<{ page_name: string }>(
        { page_name: "X", extra: "ignored" },
        { page_name: "string" }
      )
    ).toEqual({
      page_name: "X",
